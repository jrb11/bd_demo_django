...
def product_registration(request):
        ...
        if request.method == 'POST':
            ...
            #Product DB Entry
            product = Product.objects.create(Product_Name=Product_Name, Product_Image=Product_Image, Product_Sell_Price=Product_Sell_Price, Stock_Unit=Stock_Unit, Create_Date=Create_Date, Created_By_User=product_user_get)
            product.save()
...
def seller_product_view(request):
    ...
    return render(request,'seller_product_view.html',{'product_data':user_product_obj, 'Email_ID':Email})
...
def buy_product(request, pk):
    ...
    if request.method == 'POST':
        ...
        if login_user_obj.Balance >= total:
            return render(request,'buy_product.html',{'buy_data': buy_data, 'total':total, 'quntity':Unit, 'price':price, 'Email_ID':Email})
            ...
            return render(request,'buyer_product_view.html',{'product_data':product_data, 'Email_ID':Email})
            #return render(request,'buy_product.html',{'buy_data': buy_data, 'total':total, 'quntity':Unit, 'Email_ID':Email})
...
def user_purchase(request):
    ...
    if request.method == 'POST':
        ...
        if purchase_by_user_obj.Balance >= total:
            #print('True')
            return render(request,'user_purchase.html',{'purchase_user_obj':purchase_by_user_obj, 'user_buy_product_name':user_buy_product_name,'payable_amount':total, 'balance':balance,'Email_ID':Email})
        else:
            print('False')
...
def order_history(request):
    ...
    return render(request,'order_history.html',{'order_history':order_history_new, 'Email_ID':Email})
